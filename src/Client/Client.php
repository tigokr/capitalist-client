 ...
 * @version 0.7
 * @see https://github.com/capitalist-net/php-api-samples
 */
class Client implements VerificationTypes, Operations
{
    public const FORMAT_CSV = 'csv',                       // устаревший
        FORMAT_JSON = 'json',                    // рекомендуемый
        FORMAT_JSONLITE = 'json-lite';              // устаревший

    public const EMAIL_CONFIRM_TYPE_ACTIVATION = 1,
    ...
    public const NOTIFICATION_LANG_RU = 'ru',
    ...
    {
        return (new Encrypter($attributes['modulus'], $attributes['exponent']))->encrypt($password);
    }
     ...
     * @throws Exception
     */
    public function pushBatchAdvanced($batchContent, $accountRUR, $accountEUR, $accountUSD, $accountBTC, $verificationType, $verificationData = null)
    {
     ...
     *
     * @param $channel = self::NOTIFICATION_CHANNEL_EMAIL
     * @return array
     * @throws Exception
     */
    public function addPaymentNotification($document, $channel, $address, $language = self::NOTIFICATION_LANG_RU)
    {
    ...
    public function getHistory($account, $from = null, $to = null, $docState = null, $limit = 30, $page = 1)
    {
        ...
        if (!$this->sendPost($this::OPERATION_GET_HISTORY_DEPRECATED, [
            $this->passwordKey => $this->password,
            'token' => $this->token,
            'account' => $account,
            'period_from' => $from,
    ...
    protected function sendPost($operation, $params = [], $anonymous = false)
    {
        ...
        $this->setLastResponseHeaders($this->parseHttpHeaders(mb_substr($response, 0, $header_size)));
